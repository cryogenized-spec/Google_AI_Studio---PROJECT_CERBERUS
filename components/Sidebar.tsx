...
import { Plus, Trash2, Settings, MessageSquare, Menu, X, Download, Radio, ScrollText, Skull, Layers, Users, FileText, ChevronDown, ChevronRight } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const ritualThreads = threads.filter(t => t.type === 'ritual' || !t.type); 
  const reportThreads = threads.filter(t => t.type === 'report');
          ...
          {reportThreads.length > 0 && (
            <div className="mb-2">
              <button 
                ...
                className="w-full flex items-center justify-between px-2 text-[10px] text-indigo-400 uppercase tracking-widest mb-1 mt-2 font-bold hover:text-white"
              >
                <span>AI Reports ({reportThreads.length})</span>
                {reportsExpanded ? <ChevronDown size={12}/> : <ChevronRight size={12}/>}
              </button>
              
              {reportsExpanded && reportThreads.map(thread => (
                <div
                  key={thread.id}
                  className={`
                    group flex items-center gap-3 p-3 rounded-md cursor-pointer transition-all duration-200
